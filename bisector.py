...
def bisector(rv, ccf, doplot = False, low_high_cut = 0.1, showplot=False):
    ...
    # get midpoint
    bisec = (spline2(depth)+spline1(depth))/2
    ...
    if doplot:
        ...
        plt.plot(bisec,depth,label = 'bisector')
        plt.plot((bisec-np.mean(bisec))*100+np.mean(bisec),depth, label = 'bisector * 100')
        plt.legend()

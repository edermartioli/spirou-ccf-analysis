...
def get_object_rv(ccf_files,
    ...
    batch_name = '{0}/{1}__{2}__{3}'.format(outdir, collection_key, bandpass, method)

    # Generate file name for output RV time series
    output_rv_timeseries = '{0}.rdb'.format(batch_name)
    ...
    keywords = ['MJDATE','BERV','BJD', 'RV_DRIFT','EXTSN035','AIRMASS','TLPEH2O','TLPEOTR','RV_WAVFP','RV_SIMFP','DATE','MJDMID','DATE-OBS','EXPTIME']
        ...
        for i in range(len(ccf_files)):
            try:
                ...
                fit = np.polyfit(depth[(depth > bis_min) & (depth < bis_max)] - (bis_min + bis_max) / 2,
                           bis[(depth > bis_min) & (depth < bis_max)], 1, showplot=showplots)

                tbl['RV'][i] =  fit[1]
                tbl['BIS_SLOPE'][i] =  fit[0]
                tbl['BIS_WIDTH'][i] = np.mean(width[(depth > bis_min) & (depth < bis_max)])
            except:
                print('WARNING: We had an error with file {0} computing the bisector'.format(ccf_files[i]))
                print('WARNING: Values will be reported as NaN')
                tbl['RV'][i] =  np.nan
    ...
    bjd = tbl['BJD'] + (tbl['MJDMID'] - tbl['MJDATE'])
    
    save_rv_time_series(output_rv_timeseries, bjd, tbl['RV'], tbl['ERROR_RV'], time_in_rjd=True, rv_in_mps=False)

    return tbl


def create_collections(ccf_files) :
    """
